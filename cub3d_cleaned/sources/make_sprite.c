...
/*   Updated: 2021/02/23 00:06:45 by spark            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sprite_cast(t_set *s)
{
	...
	while (i < s->minfo.num_sprite)
	{
		...
		// while (x < s->spr.drawEndX)
		// {
		// 	s->spr.texX = (int)((256 * (x - (-s->spr.sp_sc_W / 2 + s->spr.spriteScreenX)) * TEX_WIDTH / s->spr.sp_sc_W) / 256);
		...
		// 		while (y < s->spr.drawEndY)
		// 		{
		// 			s->spr.d = (y - s->spr.vMoveScreen) * 256 - s->minfo.s_height * 128 + s->spr.sp_sc_H * 128;
		// 			s->spr.texY = ((s->spr.d * TEX_HEIGHT) / s->spr.sp_sc_H) / 256;

...
/*   Updated: 2021/02/26 23:27:26 by spark            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_map(t_set *set)
{
	...
	set->spr.sprt[0].y = (int)(set->minfo.m_width / 3);
	set->spr.sprt[0].x = (int)(set->minfo.m_height / 3);
	set->map2[(int)set->spr.sprt[0].x][(int)set->spr.sprt[0].y] = 3;
	set->spr.sprt[0].texnum = 4;
	
	set->spr.sprt[1].y = (int)(set->minfo.m_width / 3 + 1);
	set->spr.sprt[1].x = (int)(set->minfo.m_height / 3 + 1);
	set->map2[(int)set->spr.sprt[1].x][(int)set->spr.sprt[1].y] = 3;
	set->spr.sprt[1].texnum = 4;

	set->spr.sprt[2].y = (int)(set->minfo.m_width / 2 + 2);
	set->spr.sprt[2].x = (int)(set->minfo.m_height / 2 + 2);
	set->map2[(int)set->spr.sprt[2].x][(int)set->spr.sprt[2].y] = 3;
	set->spr.sprt[2].texnum = 4;

	set->spr.sprt[3].y = (int)(set->minfo.m_width / 2 + 3);
	set->spr.sprt[3].x = (int)(set->minfo.m_height / 2 + 3);
	set->map2[(int)set->spr.sprt[3].x][(int)set->spr.sprt[3].y] = 3;
	set->spr.sprt[3].texnum = 4;

	set->spr.sprt[4].y = (int)(set->minfo.m_width / 2 + 4);
	set->spr.sprt[4].x = (int)(set->minfo.m_height / 2 + 4);
